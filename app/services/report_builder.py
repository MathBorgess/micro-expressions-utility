...
obrigatórios ou contiver frases proibidas, refaz uma única vez com ``temperature=0.2``
(mais determinística).
"""
...
def generate_validated_report(
    *, prompt: str, generator: ReportGenerator, retry_temperature: float = RETRY_TEMPERATURE
    ...
    if validate_report(markdown, forbidden_phrases=FORBIDDEN_EXAMPLE_PHRASES):
    ...
    return ReportOutput(
        markdown=retried,
        valid=validate_report(retried, forbidden_phrases=FORBIDDEN_EXAMPLE_PHRASES),
        attempts=2,
    )
